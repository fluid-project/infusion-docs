...
a call to [fluid.defaults](../CoreAPI#fluiddefaultsgradename-options), which registers the component's defaults with the
Framework.
...
process than the default ones. (Further reading: [Options Merging](../OptionsMerging))
...
[Component Grades](../ComponentGrades)). These options have fixed names that the Framework expects
...
(Further reading on special options: [Component Configuration Options](../ComponentConfigurationOptions))

## Subcomponents

In addition to simple configurations, a components defaults are used to identify its subcomponents,
as described in [Understanding Infusion Components](../UnderstandingInfusionComponents).
