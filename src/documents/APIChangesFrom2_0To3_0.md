...
A prefsEditor is configured with the [`fluid.remoteModelComponent`](RemoteModelAPI.md) grade. This adds a `local` and `remote` model path for managing model changes between the component and those stored elsewhere. Any model value that should be persisted must be relayed into the `local` model path.
