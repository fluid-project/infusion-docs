...
In the previous [Tutorial - View Components](Tutorial-ViewComponents.md) currency converter example, you can notice two things:
...
We can clean up the implementation of our component from the [Tutorial - Evented Components](Tutorial-EventedComponents.md) still further by writing these event handlers also in a declarative form. The display update can be handled by a `modelListeners` entry of the kind we have seen before:
